...
*    Added a support program (templateParser.py) to make creating the python files and updating the README links easier.
*    Modified the README (removed things not needed by me, removed non-ASCII).
*    Removed .gitignore.
...
[**Reverse a String**](/Text/reverseString.py) - Enter a string and the program will reverse it and print it out.
...
[**Quiz Maker**](/Files/quizMaker.py) - Make an application which takes various questions form a file, picked randomly, and puts together a quiz for students. Each quiz can be different and then reads a key to grade the quizzes.
